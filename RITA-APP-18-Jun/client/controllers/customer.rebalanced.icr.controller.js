(function () {
	...
	//Methods for Rebalanced report with Bar chart	
		function displayRebalICRReport(customerID,customerName) {
	...
	function populateIncreamentalInvestment(){
		angular.forEach($scope.custRebalICRReport, function(obj) {
			obj.incrementalInvest=(obj.originalAllocPercentage-obj.currentAllocPercentage)*$scope.getRebalAllocationTotal();
	...
	*/
	$scope.getRebalAllocationTotal=function(){
      ...
      function populateSecurityId(){
              angular.forEach($scope.recommendedIAR, function(obj) {
	 ...
	 $scope.prepareRebalancedICRdata=function() { 	
	  $scope.rebalancedDataForChart=[];
		...
		angular.forEach($scope.custRebalICRReport, function(obj) {
			...
			rebalancedAllocValues.push([obj['_id'].IndustrySegment,obj.rebalancedAlloc]);
		});  
		   
		$scope.rebalancedDataForChart.push({"key": "Original Allocation","values":originalAllocValues});		
		$scope.rebalancedDataForChart.push({"key": "Current Allocation","values":currentAllocValues});
		$scope.rebalancedDataForChart.push({"key": "Rebalanced Allocation","values":rebalancedAllocValues});
	...
	var colorArray = ['#FF0000', '#0000FF', '#FFFF00', '#00FFFF'];
	  ...
	  function displayRecommendedReports(customerID,customerName) {
			var securityNames = [];
			$scope.recommendedICR =[];
			DisplayPnLService.getPnLReport(customerID,function (response) {
				...
				$scope.recommendedICR.push({"securityID":el._id['SecurityID'],"securityName":el._id['SecurityName'],
				"currentMarketPrice":el._id['FURD_Avg_CostPrice'],"recommendation":""
				,"originalAllocPercentage":originalAllocPercentage,"currentAllocPercentage":currentAllocPercentage,
				"rebalancedAlloc":rebalancedAlloc,"incrementalInvest":"","recommendation":""});
	...
	/*	
	$scope.getRecommInvestmentTotal=function(){
         if(angular.isDefined($scope.custPnLReport)){  
           var investmentTotal = 0 ;     
           angular.forEach($scope.custPnLReport,function(el) {
               if(el.incrementalInvest >0){
	...
	*/
    ...
    function handleANService() {
            AlchemyNewsService.getAlchemyNewsURLs(function (response) {
